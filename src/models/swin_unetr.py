...
def get_medopenseg(
    ...
    if pre_trained_weights:
        if os.path.exists(pre_trained_weights):
            print(f"[INFO] Loading pre-trained weights from {pre_trained_weights}")
            weights = torch.load(pre_trained_weights, map_location=device)
            model.load_from(weights)
        else:
            print(f"[WARNING] Weight file not found: {pre_trained_weights}")
